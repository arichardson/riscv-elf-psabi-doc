...
** The aggregate consists of one capability type and another type that fits into `XLEN` bits (e.g. a pointer and a `long`).
...
Each variadic argument is passed in a `YLEN`-sized stack slot.
+
...
The threshold depends on the ISA but objects smaller than 512 bytes are never affected.
+
