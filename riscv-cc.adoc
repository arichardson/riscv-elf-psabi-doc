...
* Aggregates (as opposed to scalars) up to `2*YLEN` bits are passed in `YLEN` argument register(s) if they are at least `YLEN`-bit aligned using the memory layout of the aggregate.
+
...
* Unions follow the same rules as aggregates: if it is `YLEN`-bit aligned and less than or equal to `2*YLEN`-bits in size, it is passed in memory layout in `YLEN` registers. Otherwise, the base calling convention rules apply.
