...
where each frame links to its caller using a "frame record".
A frame record consists of two pointer-sized values on the stack; the return address and the link to the next frame record.
The frame pointer register will point to the innermost frame, thereby starting the linked list.
By convention, the lowest pointer value shall point to the previous frame, while the next pointer value shall be the return address.
The end of the frame record chain is indicated by a `NULL` pointer appearing as the next link in the chain.
...
// NOTE: Using sizeof(void *) instead of XLEN/8 here is an ABI change for
// RV64ILP32 since fp is now 4 instead of 8 byte aligned. If this is a problem,
// rather than a bugfix, then we can use max(XLEN/8,sizeof(void *)) instead.
This puts the return address at `fp - sizeof(void *)`, and the previous frame pointer at `fp - 2 * sizeof(void *)`.
...
These calling conventions are used when the Y extension is present and a pure-capability ABI is in use.
...
+
NOTE: For non-pointer scalar types the maximum size that is passed in a single argument register is still `XLEN` rather than `YLEN`.
This avoids the implementation difficulty of passing `i64` in a single register when `YLEN==64` (RV32).
...
// FIXME: maybe we should just pass int128 in a single reg? Are those ever so perf-critical that the one extra YHIR/YHIW hurts?
A future calling convention variant may allow for packing these types in a single register.

// FIXME: should we simplify this and just always pass aggregates up to 2*YLEN in packed registers?
* For aggregates > `2*XLEN` but <= `2*YLEN`, pairs of registers can be used in the following cases:
** The aggregate consists of two capability types
** The aggregate consists of one capability type and another type that fits into `XLEN` bits (e.g. a pointer and a `long`), in either order.
* `YLEN`-wide aggregates (as opposed to scalars) are passed in a single `YLEN` argument register if they contain at least one pointer type.
// FIXME: is the next statement true or do we do the same splitting?
* For aggregates that can be passed in pairs of registers, they are never split into one register part and one on-stack part, but instead the entire aggregate is passed on the stack.
* Aggregates larger than `2*YLEN` or with an alignment larger than the stack alignment are passed by reference.
* A union is passed in a single register if all of its possible members can be passed in a single `YLEN` register.
Otherwise, the union is passed on the stack.
+
NOTE: This allows types such as `union { void* a; long b; }` to be passed in registers.
However, it avoids the complexity of larger aggregates where members might have conflicting register and memory layouts.
For example, in `union { struct { void* a1; long a2; } a; struct { long b1; long b2; } b; }`, the second member would normally be passed in two registers according to base aggregate rules.
Passing such a union in registers would require packing multiple members into a single `YLEN` register to ensure spilling to memory retains the correct layout, which is difficult to implement.
+
// TODO: In theory we could allow this for unions that require two registers if the in-register and in-memory layout matches for all possible union members.
...
This calling convention is the same as the IL32PC64 calling convention,
...
Registers x16-x31 do not participate in the calling convention,
so there are only six argument registers, a0-a5,
only two callee-saved registers, s0-s1,
and only three temporaries, t0-t2.

If used with an ISA that has any of the registers x16-x31 and f0-f31,
then these registers are considered temporaries.
