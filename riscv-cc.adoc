...
Any integer operation (e.g., add/multiply/etc.) writing to the `x1`-`x31` registers sets the lower XLEN bits and zeroes the upper ones.
...
* `2*XLEN` (i.e. `YLEN`) wide scalars are passed in a single argument register if then are of pointer type (including `(u)intptr_t`).
+
NOTE: For non-pointer scalar types the maximum size that is passed in a single argument register is still `XLEN` rather than `YLEN`, as the high bits of capability registers cannot be used for computation directly and need to be copied to another register using `YHIR` first.
...
In particular, this calling convention must not be used with the Q ISA extensions.
