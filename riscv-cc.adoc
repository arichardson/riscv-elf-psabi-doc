...
at `fp - sizeof(void *)`, and the previous frame pointer at `fp - 2 * sizeof(void *)`.
...
The register names and usage remain the same as the integer calling convention, but the registers `x0`-`x31` (and their ABI aliases) now refer to the `YLEN`-wide extended GPRs.
Any integer operation (e.g., add/multiply/etc.) writing to the ``x0`-`x31` registers sets the lower XLEN bits and zeroes the upper ones.

The pure-capability ABIs follow the same calling conventions as the base integer and hardware floating-point calling conventions, with the distinction that pointers are replaced by capabilities.
This means that arguments and return values are passed in the same manner, but any pointer types are instead treated as capabilities.
...
1. All variadic arguments (those corresponding to the `...` in the function prototype) are passed on the stack. They never occupy argument registers `a0`-`a7`.
2. A capability that covers all on-stack arguments on the stack is passed in the `x5` (`t0`) register.
3. This capability has its Write and Execute permissions cleared.
...
The IL32PC64E calling convention is not compatible with ISAs that have registers that require load and store alignments of more than 64 bits.
In particular, this calling convention must not be used with the D or Q ISA extensions.
...
(i.e. ELFCLASS32, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_SOFT,EF_RISCV_FLOAT_ABI_SOFT>>).

[[abi-il32pc64f]]
IL32PC64F:: IL32PC64 with hardware floating-point calling convention for FLEN=32
(i.e. ELFCLASS32, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_SINGLE,EF_RISCV_FLOAT_ABI_SINGLE>>).

[[abi-il32pc64d]]
IL32PC64D:: IL32PC64 with hardware floating-point calling convention for FLEN=64
(i.e. ELFCLASS32, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_DOUBLE,EF_RISCV_FLOAT_ABI_DOUBLE>>).
...
(i.e. ELFCLASS32, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>>, <<EF_RISCV_FLOAT_ABI_SOFT,EF_RISCV_FLOAT_ABI_SOFT>>, and <<EF_RISCV_RVE,EF_RISCV_RVE>>).

[[abi-l64pc128]]
L64PC128:: Integer and capability calling convention only, hardware floating-point calling convention is not used
(i.e. ELFCLASS64, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_SOFT,EF_RISCV_FLOAT_ABI_SOFT>>).

[[abi-l64pc128f]]
L64PC128F:: L64PC128 with hardware floating-point calling convention for FLEN=32
(i.e. ELFCLASS64, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_SINGLE,EF_RISCV_FLOAT_ABI_SINGLE>>).

[[abi-l64pc128d]]
L64PC128D:: L64PC128 with hardware floating-point calling convention for FLEN=64
(i.e. ELFCLASS64, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_DOUBLE,EF_RISCV_FLOAT_ABI_DOUBLE>>).

[[abi-l64pc128q]]
L64PC128Q:: L64PC128 with hardware floating-point calling convention for FLEN=128
(i.e. ELFCLASS64, <<EF_RISCV_CHERIABI,EF_RISCV_CHERIABI>> and <<EF_RISCV_FLOAT_ABI_QUAD,EF_RISCV_FLOAT_ABI_QUAD>>).
...
the C standard library on CHERI-RISC-V defines `intcap_t`/`uintcap_t` types that can hold capabilities (YLEN values) and integers (XLEN values) alike.
...
As `intptr_t`/`uintptr_t` must hold pointers and integers, they mapped to `intcap_t`/`uintcap_t` in the pure-capability ABIs.
All these types must be aligned to YLEN bits.
+
