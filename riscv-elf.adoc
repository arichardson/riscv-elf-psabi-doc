...
[[EF_RISCV_CHERIABI]]
EF_RISCV_CHERIABI (0x00000040)::: This bit is set when the binary targets the
...
| PT_RISCV_MEMTAG_CHERI | 0x7fffffff | Capability memory tags
// TODO: Should probably assign a different value here.
|===
...
* `p_paddr`: 0
* `p_filesz`: segment size in file, calculated as `p_memsz / (8 * sizeof(intcap_t))` (eight 1-bit tags cover 8 capabilities of memory)
...
=== Section and Segment Alignment for pure-capability ABIs
...
segment that do not overlap with other loadable segments. See <<riscv-cheri-spec,the CHERI Specification>> for
