...
// NOTE: Previously used the custom value 0x00010000
[[EF_RISCV_RVY]]
EF_RISCV_RVY (0x00000040)::: This bit is set when the binary targets the
pure capability ABIs defined by this specification.


Until such a time that the *Reserved* bits (0x00ffff80) are allocated by future
...
For pure-capability ABIs, additional processor-specific segment types are
defined by the <<cheri-gelf-abi,CHERI Generic ELF ABI>>.
...
For pure-capability ABIs, additional processor-specific note sections are defined
by the <<cheri-gelf-abi,CHERI Generic ELF ABI>>.
Otherwise, there are no RISC-V specific definitions relating to ELF note
sections.
...
For pure-capability ABIs, additional processor-specific dynamic array tags
are defined by the <<cheri-gelf-abi,CHERI Generic ELF ABI>>.
...
* [[[cheri-gelf-abi]]] "CHERI Generic ELF ABI"
https://github.com/CTSRD-CHERI/cheri-elf-gabi/blob/main/gabi.md
